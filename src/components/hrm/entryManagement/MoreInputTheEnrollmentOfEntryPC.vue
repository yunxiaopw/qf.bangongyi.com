    ...
    <div class="more_input">
        ...
        <div v-if="i_item.type === 'radio'">
            <el-radio-group :value="value" v-bind="$attrs" v-on="$listeners" :readonly="i_item.readonly" size="small">
                <el-radio v-for="(item, key) in i_item.value" :key="key" :label="key">{{item}}</el-radio>
            </el-radio-group>
        </div>
        <!-- 多选 -->
...
<script>
import { rankLevel, widgetPosition, rank_Level, widgetArea } from "@/api/hrm/entryManagement/theEnrollmentOfEntryPC";
import { mapState ,mapMutations } from "vuex"

import { getUrlParams } from '@/utils/utils'

export default {
  name: "MoreInputTheEnrollmentOfEntryPC",
  props: {
    // 组件配置信息
    i_item: {
      type: Object,
  ...
  data() {
    return {
        ...
        number: 0,
        uploadList: [],
        baseUrl: process.env.NODE_ENV == 'development' ? '/hrm/openapi/widget/uploadfile' : process.env.API_HOST + '/hrm/openapi/widget/uploadfile',
        uploadFileUrl: process.env.NODE_ENV == 'development' ? '/hrm/openapi/widget/uploadfile' : process.env.API_HOST + '/hrm/openapi/widget/uploadfile', // 上传的图片服务器地址
        headers: {
          token: getUrlParams('t'),
