<template>
  ...
  >
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <div>
        <el-button class="btnClearAll" @click="deleteSelectListItem('all')">清空已选</el-button>
        <el-button class="greenBtn" type="primary" @click="ackSelect">确定</el-button>
      </div>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    ...
    ackSelect(){
      if(this.radio && this.selectList.length > 1) {
        ...
        this.listWidgetHide();
        this.$emit("ackSelect", { selects: this.selectList });
      }
    },
    // 树形点击获取右侧列表
    getList(data){
